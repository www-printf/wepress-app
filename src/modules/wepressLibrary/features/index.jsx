...
import { Select, Pagination, Spinner, TextInput } from "flowbite-react";
import { HiViewGrid, HiViewList, HiArrowUp, HiArrowDown, HiEye, HiDownload, HiDotsVertical, HiSearch } from "react-icons/hi";

const fetchDocuments = async ({ page, per_page, sort, order }) => {
    const response = await fetch(
        `/api/documents?page=${page}&per_page=${per_page}&sort=${sort}&order=${order}`
    );
    if (!response.ok) {
                        ...
                        >
                            {viewMode === "list" ? (
                                <>
                                    ...
                                    <div className="flex-1">
                                        <span className="text-sm text-gray-600">Thứ {doc.created_day}</span>
                                    </div>
                                    <div className="flex-1">
                                        <span className="text-sm text-gray-600">{doc.publish_year}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                ...
                                <>
                                    ...
                                    <div className="flex justify-between items-center mt-2 text-xs text-gray-500">{"Tác giả: " + doc.author}</div>
                                    <div className="flex justify-between items-center mt-2 text-xs text-gray-500">{"Phiên bản: " + doc.version}</div>
                                    <div className="flex justify-between items-center mt-2 text-xs text-gray-500">{"Năm xuất bản: " + doc.publish_year.slice(0, 4)}</div>
                                </>
                            )}
                        </div>
                    ))}
