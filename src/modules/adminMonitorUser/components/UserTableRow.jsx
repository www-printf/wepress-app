...
export function UserTableRow({ user }) {
    return (
      <div className="flex flex-wrap items-center bg-white border-r border-b border-l border-zinc-200 max-md:max-w-full">
        ...
        <div className={`gap-1.5 self-stretch p-2.5 my-auto text-sm w-[200px] ${user.status === 'active' ? 'text-green-500' : 'text-amber-400'}`}>
          {user.status === 'active' ? 'Đang hoạt động' : 'Đang đình chỉ'}
        </div>
        <Link to={ENDPOINTS.ADMIN.USER_DETAIL.replace(':userID', user.id)} className="flex gap-1.5 justify-center items-center self-stretch p-2.5 my-auto w-[130px]">
          <img
            loading="lazy"
